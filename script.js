...
/* ============================================================
   SHANE — READING HIS JOURNAL  (cozy Stardew-style page-flip)
   ------------------------------------------------------------
   A warm pixel room. Shane's journal sits open on a desk; little
   pixel Shane is nearby, mostly looking away. Tap "Turn Page" to
   read entries (page-flip). Now and then he STIRS (warning) then
   GLANCES OVER — tap "Shut it!" before he sees, or you're caught.
   Reading more raises a quiet "deep read" meter that improves the
   odds of rarer entries; luck still rolls every page.
   Tiers: common / uncommon / rare(embarrassing) / buried(past) /
   legendary(Eurielle). Only initialises if #sjGame exists.
   ============================================================ */
(function initShaneJournalGame() {
  function build() {
    var root = document.getElementById('sjGame');
    if (!root || root._init) return;
    root._init = true;

    var canvas   = document.getElementById('sjCanvas');
    var tagEl    = document.getElementById('sjTag');
    var entryEl  = document.getElementById('sjEntry');
    var dateEl   = document.getElementById('sjDate');
    var stickerEl= document.getElementById('sjSticker');
    var turnBtn  = document.getElementById('sjTurn');
    var shutBtn  = document.getElementById('sjShut');
    var gazeEl   = document.getElementById('sjGaze');
    var gazeTx   = document.getElementById('sjGazeText');
    var readCt   = document.getElementById('sjReadCount');
    var rareCt   = document.getElementById('sjRareCount');
    var overlay  = document.getElementById('sjOverlay');
    var calloutEl= document.getElementById('sjCallout');
    var subEl    = document.getElementById('sjSub');
    var startBtn = document.getElementById('sjStart');
    var lineEl   = document.getElementById('sjLine');
    if (!canvas || !turnBtn) return;
    ...
    var DATES = [
      ...
      "\u2014 no idea what day it is", "\u2014 written in the dark", "\u2014 gas station lot",
      "\u2014 motel off the interstate", "\u2014 back of the venue", "\u2014 4 a.m., everyone asleep",
      "\u2014 the long drive north", "\u2014 nobody around", "\u2014 hands not steady today",
      "\u2014 didn't date this one on purpose", "\u2014 somewhere in Nevada", "\u2014 load-out done"
    ];

    /* ---- COMMON: lyrics, annoyances, mundane tour life, riff ideas ---- */
    var COMMON = [
      { tag: 'Annoyance', t: "Fuck this hotel wifi. Buffering on a THREE MINUTE video. I've toured four continents and this is what breaks me." },
      { tag: 'Annoyance', t: "Cody used my toothbrush. AGAIN. Says he 'thought it was the spare.' There is no spare. There is only mine, and now his betrayal." },
      { tag: 'Annoyance', t: "Whoever keeps moving my pick to the left side of the case: I will find you. I have the time. I have the patience. I have a list." },
      { tag: 'Annoyance', t: "The venue 'green room' was a janitor's closet with a folding chair. Scorch called it 'cozy.' Scorch would call a dumpster cozy." },
      { tag: 'Annoyance', t: "Drive-thru forgot my fries. Drove 40 minutes back. They were closed. The universe owes me fries." },
      { tag: 'Annoyance', t: "Someone set the bus thermostat to 61. It's a meat locker. I can see my breath. I'm writing this in gloves." },
      { tag: 'Annoyance', t: "New amp tech tuned my bass a half step down 'to be safe.' Safe from WHAT. Music?" },
      { tag: 'Annoyance', t: "Ran out of coffee at a truck stop that only had decaf. Decaf. They looked me in the eye and offered me decaf." },
      { tag: 'Annoyance', t: "Phone autocorrected 'soundcheck' to 'soundcheese' in the group chat and now that's the band name forever, apparently." },
      { tag: 'Annoyance', t: "The hotel pillow is somehow both a brick and a marshmallow. Engineering should study it. I should sleep on the floor." },
      { tag: '\u266A Lyric', t: "low note holds the room together\nnobody claps for the floor\nbut the floor's still there\nwhen the lights hit the door" },
      { tag: '\u266A Lyric', t: "midnight hums in the key of leaving\nand I never learned the words\njust the spaces in between them\nwhere the quiet gets to hurt" },
      { tag: '\u266A Lyric', t: "the quiet ones keep the time\nso the loud ones get to fall\nI'll be down here in the bottom end\ncatching all of y'all" },
      { tag: '\u266A Lyric', t: "four strings and a closed door\nthat's the whole song, there ain't more\nbut you hum it anyway\nand I let the bassline stay" },
      { tag: '\u266A Lyric', t: "I carry the low end like a debt\nplay it slow, never paid it off yet\none day the room'll go quiet enough\nto hear what I never said" },
      { tag: '\u266A Lyric', t: "she walks in on the downbeat\nthe whole bar holds its breath\nI miss the chord change every time\nworth it, nonetheless" },
      { tag: 'Riff Idea', t: "Riff idea — drop D, walk it: D / / / | F / G / | A (hold, let it ring 2 bars). Slides on the way up, dead note on the way down. Greasy. Yes." },
      { tag: 'Riff Idea', t: "Bass tab, chorus thing:\nG|--------------------|\nD|--------------------|\nA|--5-5--7--5-3-------|\nE|-3----3-----3--3-1-|\nslow it WAY down. let it breathe. Cody'll want to rush it. don't let him." },
      { tag: 'Riff Idea', t: "Idea: whole bridge on ONE note. Just rhythm. Let Scorch scream over it. Sometimes the bravest thing a bassline does is refuse to move." },
      { tag: 'Riff Idea', t: "Walking line in 6/8, minor pentatonic, but land on the major third once per cycle — that one wrong-right note that makes people lean in. Steal joy where you can." },
      { tag: 'Riff Idea', t: "Note to self: the song doesn't need a solo. The song needs a hole where a solo would go. Leave it empty. Let them want it." },
      ...
      { tag: 'Mundane', t: "Fixed the rattle in the van door with a folded gum wrapper. Nobody noticed. That's the job. Most of life is folded gum wrappers." },
      { tag: 'Mundane', t: "Did laundry in a gas station sink. Don't ask. It worked. Mostly. My socks smell like blue raspberry now." },
      { tag: 'Mundane', t: "Counted 14 water bottles rolling around the bus. We buy water. We lose water. We are a band, and the bus is a tide." },
      { tag: 'Bike', t: "Knucklehead idling rough again. Pulled the plugs. They're fine. SHE'S just dramatic. Like everything I love." },
      { tag: 'Bike', t: "Rode two hours just to ride back. Didn't go anywhere. Went everywhere. You get it or you don't." },
      { tag: 'Bike', t: "Cleaned the chrome till I could see my own dumb face in it. Cheaper than therapy. Louder, too." },
      { tag: '3 A.M.', t: "Rain on the glass sounds like brushes on a snare. Couldn't sleep. Wrote that down instead of sleeping. Classic me." },
      { tag: '3 A.M.', t: "Hotel ceiling. Counted the cracks. Lost count. Started over. This is the most productive I've been all week." },
      { tag: '3 A.M.', t: "Can hear Cody snoring through the wall. Weirdly, it helps. Means he's still here. Means we all made it to another city." }
    ];

    /* ---- UNCOMMON: band texture, tour stories, Kayla/Max/Cody ---- */
    var UNCOMMON = [
      { tag: 'The Band', t: "Cody had a bad night. Sat with him on the curb till the shaking stopped. Didn't say much. Didn't have to. Some friends you keep with silence." },
      { tag: 'The Band', t: "Scorch is louder than his demons most days. Some days he isn't. Those days I stay close and pretend I'm just bored. He doesn't know I clock it. Good." },
      { tag: 'The Band', t: "Max thinks nobody sees how scared he is of all this ending. I see it. I'd never say it. That's why he trusts me with the quiet parts." },
      { tag: 'The Band', t: "Kayla left a sticky note on my case: 'you're not as scary as you think.' Kept it. It's in the lining now. Won't tell her." },
      { tag: 'Tour', t: "Played a town so small the venue was a barn. Best crowd in months. Turns out people who drive an hour to see you actually listen." },
      { tag: 'Tour', t: "Border crossing took five hours because Scorch's passport photo 'looked like a different man.' It's three years old. He's just aged like a struck match." },
      { tag: 'Tour', t: "The merch table sold out of everything but the XXL shirts and one lonely tote bag. Kayla bought the tote out of pity. It says SNAKE SKINS. She uses it for snacks now." },
      { tag: 'Tour', t: "Got recognized at a rest stop in the middle of nowhere. Kid had our logo on his backpack. Signed it. Drove off feeling ten feet tall. Don't tell the band." },
      { tag: 'Tour', t: "Generator died mid-set in the desert. We finished the last three songs acoustic, by phone flashlight, crowd singing the bass parts. Best show we'll never get back." },
      { tag: 'Kayla', t: "Kayla's started leaving little drawings in the tour notebook. Today: a snake wearing a tiny crown. Captioned 'lilith, queen of the bus.' She's not wrong." },
      { tag: 'Kayla', t: "Kayla can out-stare anyone alive. Watched her win a stand-off with a customs officer through sheer goth indifference. I'd follow her into battle." },
      { tag: 'Max', t: "Max said 'heaps good' about the catering and Cody's been saying it for three days now in a bad Australian accent. Max has stopped correcting him. We've all stopped." },
      { tag: 'Max', t: "Max gets homesick around 2 a.m. and starts talking about the ocean back home. I let him. Everybody's got a tide pulling them somewhere." },
      { tag: 'Cody', t: "Cody apologized to a door he walked into. The door did not accept. Cody felt bad about it for the rest of the night. I love this idiot." },
      { tag: 'Cody', t: "Cody's the best drummer I've ever played with and he genuinely does not believe me when I say it. Told him again tonight. He said 'you're just being nice.' I have never once been nice." },
      { tag: 'About Her', t: "She laughed at the dumb one. The joke I don't tell anyone. Filed that away somewhere I keep things I'm not ready to look at." },
      { tag: 'About Her', t: "Caught myself saving the window seat. For nobody. For her. Same thing now, apparently." },
      { tag: 'About Her', t: "She hums our song wrong on purpose just to get a rise out of me. I stopped correcting her three towns ago. That's the tell, isn't it. That's the whole tell." },
      { tag: 'About Her', t: "Wrote a bassline thinking about the way she walks. Nobody will ever know that's what it is. She might. She's sharp like that." },
      { tag: 'Mom', t: "Heard a woman laugh in a diner exactly like Mom used to. Sat in the parking lot a while after. Just sat. Some sounds reach right through the years and grab you." },
      { tag: 'Mom', t: "Made her soup recipe from memory. Got it wrong. Ate it anyway. Wrong tastes better than nothing. Most days that's the whole philosophy." }
    ];

    /* ---- RARE: embarrassing band gold (Scorch-heavy) ---- */
    var RARE = [
      { tag: '\u2605 Scorch', t: "Walked in on Scorch in a full teddy-bear ONESIE. Ears on the hood. He froze like a deer. Said 'it's for circulation.' I have so many questions and I will ask none of them. The leverage alone is worth more than the tour." },
      { tag: '\u2605 Scorch', t: "Scorch's onesie has a name. He calls it 'Mr. Snuggleworth.' I heard him say goodnight to it. To the ONESIE. Our terrifying frontman tucks himself into a bear named Snuggleworth. I will take this to my grave. Or sell it. Haven't decided." },
      { tag: '\u2605 Scorch', t: "Found out Scorch is scared of moths. Not spiders. Not heights. MOTHS. One got in the bus and he stood on the table screaming 'IT HAS A FACE, IT HAS A LITTLE FACE.' Hardest I've laughed in a year." },
      { tag: '\u2605 Scorch', t: "Scorch cried at a dog food commercial. The one with the old dog waiting by the door. He blamed 'allergies.' We were in the desert. There was nothing to be allergic to except his own enormous heart, the big spiky idiot." },
      { tag: '\u2605 Cody', t: "Cody ordered 'novelty lollipops' for the merch table to be cheeky. Box arrives. They are unmistakably, anatomically DICK-SHAPED. 500 of them. He'd already paid. Kayla put them out anyway. They were our best seller in Phoenix. We don't talk about Phoenix." },
      { tag: '\u2605 Cody', t: "Cody tried to flush a whole burrito down the bus toilet 'to hide the evidence' from his diet coach. Flooded the bus. We pulled over on the interstate so Cody could confess to a plumber about a burrito. Greatest day of my life." },
      { tag: '\u2605 Cody', t: "Cody got so nervous meeting a drummer he idolizes that he introduced himself as 'Brody.' Just panicked and became Brody. The guy called him Brody all night. Cody answered to it. He's Brody now in three states." },
      { tag: '\u2605 Kayla', t: "Kayla 'doesn't do feelings' and yet I caught her hand-sewing a tiny patched blanket for Lilith's tank because she read snakes 'like to feel secure.' Threatened to end me if I told anyone. So I'm only telling this book. Hi, book." },
      { tag: '\u2605 Kayla', t: "Kayla, queen of darkness, eyeliner like a blade, has the entire discography of a bubblegum pop group memorized and sings it FLAWLESSLY in the shower. I have recordings. I will never use them. But I have them. For balance in the universe." },
      { tag: '\u2605 Max', t: "Max got so sunburned at an outdoor festival he played the whole set the color of a stop sign, in a WHITE shirt, looking like a thermometer about to blow. He kept saying 'she'll be right.' She was not right. He glowed in the dark." },
      { tag: '\u2605 Max', t: "Max tried to impress a girl by saying he 'wrestled a croc back home.' She asked what kind. He said, with total confidence, 'a big one.' She left. He's still proud of it. 'A big one,' Max. Poetry." },
      { tag: '\u2605 Embarrassing', t: "We all got matching friendship bracelets at a truck stop as a joke and now nobody will take theirs off and we won't admit it's not a joke anymore. I sleep in mine. So does Scorch. Over the onesie." },
      { tag: '\u2605 Embarrassing', t: "Band photo shoot: the photographer said 'look tough.' Cody sneezed, Scorch blinked, Max yawned, Kayla flipped off the camera, and I was looking at a duck on the lake. They used that photo. It's our best one. It's the album cover now." }
    ];

    /* ---- BURIED: the gritty past, gang life, the woman, the kid ---- */
    var BURIED = [
      { tag: '\u26A0 Buried', t: "The first one wasn't an accident and I knew it wasn't, even while I was telling myself it was. He had a name. I made myself forget it. It didn't take. It never takes." },
      { tag: '\u26A0 Buried', t: "People think the worst part is doing it. It isn't. The worst part is how normal breakfast tastes the next morning. How the coffee's still good. How you're still you, and the world just lets you keep being you." },
      { tag: '\u26A0 Buried', t: "The club had rules. You don't bury your own where they can be found, and you don't leave a debt where it can be paid. I was good at both. That's not a thing you put on a resume. It's a thing you put in a hole and stand on." },
      { tag: '\u26A0 Buried', t: "There's a stretch of county road I won't drive anymore. Not won't. Can't. The bus took a detour once and got close and my hands started shaking on a steering wheel I wasn't even holding." },
      { tag: '\u26A0 Buried', t: "A man begged once. They mostly don't, in the movies. But they do. I let him finish the sentence. I still don't know why. It didn't change what happened. It changed me. Small difference. Permanent one." },
      { tag: '\u26A0 Buried', t: "I counted, once, in a motel, just to know the number. Stopped where I stopped because past that it stops being a number and starts being a thing you carry up every flight of stairs for the rest of your life." },
      { tag: '\u26A0 Buried', t: "Her name was Rae. Before all this. Before the bass, before the band, before I learned what my hands could do. She had paint under her nails always and she laughed like she'd invented it. I loved her stupid. I left to keep her clean of what I'd become." },
      { tag: '\u26A0 Buried', t: "Rae wrote me one letter after I ran. I never opened it. Carried it eleven years. It's in the lining of the bass case right now, sealed. Some doors you keep shut because the room behind them is the only room you've got left of her." },
      { tag: '\u26A0 Buried', t: "Sometimes I do the math. If she kept it. If it was a year before I left, give or take. There could be a kid out there with my jaw and her laugh, grown now, who has no idea the low end of every Snake Skins song is a man wondering about them. I hope they got her hands. Not mine." },
      { tag: '\u26A0 Buried', t: "I don't believe in ghosts. I believe in maps. I've got a whole one in my head — every spot, every road, every hole — and I'd burn it if burning it worked. It doesn't. I've tried. The map's drawn on the inside." },
      { tag: '\u26A0 Buried', t: "Mom never knew. That's the only mercy in the whole thing. She died thinking I'd run off to be young and dumb. I let her keep that. Cheapest, kindest lie I ever told. The only good thing those years let me give her." },
      { tag: '\u26A0 Buried', t: "Somebody said my old road name in a bar last month. Whole room didn't notice. I noticed. Left before my drink came. You spend years becoming a bass player named Shane and one stranger's mouth can drag the other man right back into the chair beside you." }
    ];

    var LEGENDARY = { tag: '\u2606 ? ? ?', t: "I am in love with the Goddess Eurielle.", date: "\u2014 this page was stuck to the next one" };

    /* ---- templated filler so common/uncommon stay fresh ---- */
    function tplAnnoy() {
      return { tag: 'Annoyance', t: pick(["Cody", "Scorch", "Max", "Kayla", "Somebody", "The opener's drummer"]) + " " +
        pick(["used my towel", "ate my leftovers", "moved my pick", "left the bus door open", "borrowed my charger and vanished", "parked the bike crooked", "touched the thermostat", "hummed off-key for an HOUR", "set the alarm for 5 a.m. 'by accident'"]) +
        ". " + pick(["We'll discuss it.", "Noted in the book of grudges.", "I'm not mad. I'm worse than mad. I'm patient.", "Said nothing. Wrote this. Healthier, probably.", "There will be a reckoning. A quiet one."]) };
    }
    function tplLyric() {
      return { tag: '\u266A Lyric', t: pick(["low note holds the line", "she walks in on the downbeat", "four strings, one truth", "the bass don't ask for credit", "midnight hums in E", "the quiet ones keep time"]) +
        "\n" + pick(["nobody claps for the floor", "and the whole room leans in", "the rest is just decoration", "somebody's gotta carry the weight", "she never even noticed", "and call it a melody"]) };
    }

    var recent = [];
    function fresh(t) { return recent.indexOf(t) === -1; }
    function remember(t) { recent.push(t); if (recent.length > 10) recent.shift(); }

    /* ===========================================================
       STATE
       =========================================================== */
    var playing = false, rafId = null, lastT = 0;
    var reads = 0, rares = 0, best = 0, deep = 0;     // deep = "deep read" meter 0..100
    var gaze = 'away', gazeTimer = 0, caughtArmed = false;
    var flip = 0;                                      // page-flip animation 0..1
    var animT = 0, shaneBob = 0;
    var lastEntry = '', currentRarity = '';
    var foundEurielleEver = false, eurielleThisGame = false, buriedThisGame = 0;

    /* ---- odds improve with the deep-read meter ---- */
    function genEntry() {
      var d = deep / 100;            // 0..1
      // legendary — luck, helped a little by deep reading; once per game
      var legChance = 0.006 + d * 0.02;
      if (reads >= 5 && !eurielleThisGame && Math.random() < legChance) {
        eurielleThisGame = true;
        return { tag: LEGENDARY.tag, text: LEGENDARY.t, date: LEGENDARY.date, rarity: 'legendary' };
      }
      // buried — deeper reading meaningfully helps
      var burChance = 0.04 + d * 0.12;
      if (reads >= 3 && buriedThisGame < 3 && Math.random() < burChance) {
        buriedThisGame++;
        var b = pickFresh(BURIED);
        return { tag: b.tag, text: b.t, date: pick(DATES), rarity: 'buried' };
      }
      // rare embarrassing — luck + a little deep help
      var rareCut = 0.12 + d * 0.10;
      var uncCut  = rareCut + 0.34;
      var r = Math.random(), tier, e, useTpl = false;
      if (r < rareCut) { tier = 'rare'; e = pickFresh(RARE); }
      else if (r < uncCut) { tier = 'uncommon'; useTpl = Math.random() < 0.35; e = useTpl ? null : pickFresh(UNCOMMON); }
      else { tier = 'common'; useTpl = Math.random() < 0.45; e = useTpl ? null : pickFresh(COMMON); }
      if (!e) {
        var g = 0;
        do { e = (tier === 'uncommon') ? tplAnnoy() : pick([tplAnnoy, tplLyric])(); g++; } while (!fresh(e.t) && g < 14);
        remember(e.t);
      }
      return { tag: e.tag, text: e.t, date: pick(DATES), rarity: tier };
    }
    function pickFresh(pool) {
      var e, g = 0; do { e = pick(pool); g++; } while (!fresh(e.t) && g < 20); remember(e.t); return e;
    }

    /* ---- banter ---- */
    var LINES = {
      turn:  ["Next page\u2026", "\u2026go on.", "What else is in here.", "Keep reading.", "One more."],
      got:   ["\u2026Oh, that's a good one.", "Noted.", "Filed away.", "You shouldn't be reading this. You are.", "Yikes. Keep going."],
      rare:  ["Ha! Band gold.", "Oh, this is PRECIOUS.", "He'd die if he knew you saw this.", "Screenshot-worthy, if you were that kind of person."],
      buried:["\u2026Oh. Oh, that's heavy.", "You weren't supposed to find that.", "Some pages are graves. You just read one.", "\u2026You can't un-know that now."],
      legend:["\u2026You weren't ever supposed to read that one.", "The whole journal was hiding this."],
      stir:  ["\u2026he's stirring. Watch it.", "He shifted. Careful.", "\u2026he might look over."],
      look:  ["HE'S LOOKING \u2014 shut it!", "Eyes up \u2014 SHUT THE BOOK!", "Now! Close it!"],
      safe:  ["\u2026phew. Back to it.", "Close one. He didn't see.", "Snapped it shut just in time."],
      caught:["He saw. '\u2026The hell are you doin' in my book?'", "Caught. He closes it slow and says nothing. Worse than yelling.", "Busted. He just looks at you. That's the whole punishment."]
    };
    function say(t, k) { lineEl.textContent = t; lineEl.className = 'sj-line' + (k ? ' ' + k : ''); }
    ...
    function setGaze(state) {
      gaze = state;
      gazeEl.className = 'sj-gaze ' + state;
      if (state === 'away')        { gazeTx.textContent = pick(["He's tuning his bass.", "He's looking out the window.", "His back's to you.", "He's miles away."]); gazeTimer = rand(3200, 6000); caughtArmed = false; }
      else if (state === 'stir')   { gazeTx.textContent = "\u2026he's stirring."; gazeTimer = REDUCE ? 900 : rand(650, 1100); say(pick(LINES.stir), 'warn'); }
      else /* looking */           { gazeTx.textContent = "HE'S LOOKING!"; gazeTimer = rand(1100, 1700); caughtArmed = true; say(pick(LINES.look), 'bad'); }
    }

    function turnPage() {
      if (!playing || flip > 0) return;
      // if he's actively looking and you turn instead of shutting -> caught
      if (gaze === 'looking') { caught(); return; }
      var e = genEntry();
      currentRarity = e.rarity;
      pendingEntry = e;
      flip = 1;                        // triggers flip; entry swaps at midpoint
      say(pick(LINES.turn));
    }
    var pendingEntry = null, swapped = false;
    ...
    function applyEntry(e) {
      ...
      root.classList.remove('rare', 'buried', 'legend');
      stickerEl.hidden = true; stickerEl.className = 'sj-sticker';
      reads++; readCt.textContent = reads;
      deep = clamp(deep + 6, 0, 100);
      if (e.rarity === 'rare')      { root.classList.add('rare');   stickerEl.hidden = false; stickerEl.classList.add('rare');   stickerEl.textContent = 'BAND GOLD'; rares++; rareCt.textContent = rares; say(pick(LINES.rare), 'gold'); }
      else if (e.rarity === 'buried'){ root.classList.add('buried'); stickerEl.hidden = false; stickerEl.classList.add('buried'); stickerEl.textContent = '\u26A0 BURIED'; rares++; rareCt.textContent = rares; say(pick(LINES.buried), 'blood'); }
      else if (e.rarity === 'legendary'){ root.classList.add('legend'); stickerEl.hidden = false; stickerEl.classList.add('legend'); stickerEl.textContent = '\u2606 ? ? ?'; foundEurielleEver = true; rares++; rareCt.textContent = rares; say(pick(LINES.legend), 'gold'); }
      else { if (Math.random() < 0.5) say(pick(LINES.got)); }
    }

    function shutBook() {
      if (!playing) return;
      if (gaze === 'looking' || gaze === 'stir') {
        // saved!
        say(pick(LINES.safe), 'good');
        setGaze('away');
      } else {
        say(pick(["Nothing to hide from yet.", "He's not even looking. Relax."]));
      }
    }

    function caught() {
      playing = false; pauseLoop();
      if (reads > best) best = reads;
      calloutEl.textContent = "He Caught You.";
      subEl.innerHTML = '<span class="sj-final">Pages read <b>' + reads + '</b>' +
        '<span class="best">Best: ' + best + '  \u00B7  rare finds: ' + rares + '</span>' +
        (foundEurielleEver ? '<span class="last gold">\u2606 You found the one he\u2019ll never say out loud.</span>' : '') +
        (lastEntry ? '<span class="last">last page you saw: \u201C' + lastEntry + '\u201D</span>' : '') + '</span>';
      startBtn.textContent = 'Read Again';
      overlay.classList.remove('hidden');
      say(pick(LINES.caught), 'bad');
    }

    function start() {
      playing = true;
      reads = 0; rares = 0; deep = 0; eurielleThisGame = false; buriedThisGame = 0; recent = [];
      readCt.textContent = '0'; rareCt.textContent = '0';
      root.classList.remove('rare', 'buried', 'legend');
      stickerEl.hidden = true;
      overlay.classList.add('hidden');
      applyEntry({ tag: '\u2014', text: "(you flip open the cover\u2026)\n\ntap Turn Page to read.", date: "\u2014 his journal", rarity: 'common' });
      reads = 0; readCt.textContent = '0'; deep = 0;   // the cover doesn't count
      setGaze('away');
      say("It's open. Read while he's not looking.");
      startLoop();
    }

    /* ===========================================================
       RENDER — cozy Stardew-ish pixel room
       =========================================================== */
    var VW = 320, VH = 200;
    function px(x, y, w, h, c) { ctx.fillStyle = c; ctx.fillRect(x | 0, y | 0, w | 0, h | 0); }
    var C = {
      wallTop:'#caa37a', wall:'#b88a5e', wallLine:'#a87c52',
      floor:'#7a5436', floorLine:'#6a472c',
      rug:'#5a7a6a', rug2:'#6e8f7d',
      desk:'#8a5a36', deskTop:'#a06a40', deskDark:'#6e4628',
      book:'#f2ead4', bookEdge:'#caa24a', ribbon:'#b44a4a',
      lampGlow:'rgba(255,210,140,0.5)', lamp:'#e7c24a',
      skin:'#e8b890', hair:'#6a4226', shirt:'#3c4658', shirtHi:'#4c576a',
      plant:'#4a8a5a', pot:'#b5654a', window:'#7ec0e0', sky:'#bfe3f2',
      catO:'#e0954a', catD:'#1a1a1a'
    };

    function draw() {
      // wall
      px(0, 0, VW, 120, C.wall);
      px(0, 0, VW, 8, C.wallTop);
      for (var wx = 0; wx < VW; wx += 16) px(wx, 8, 1, 112, C.wallLine);
      // floor
      px(0, 120, VW, VH - 120, C.floor);
      for (var fx = 0; fx < VW; fx += 16) px(fx, 120, 1, VH - 120, C.floorLine);
      px(0, 120, VW, 2, '#8a6446');
      // rug under the desk
      px(70, 150, 180, 40, C.rug);
      px(76, 156, 168, 28, C.rug2);

      // window (left) with daylight
      px(20, 26, 52, 44, '#5a3c24');
      px(24, 30, 44, 36, C.sky);
      px(24, 30, 44, 36, C.window); px(24, 30, 44, 18, C.sky);
      px(45, 30, 2, 36, '#5a3c24'); px(24, 47, 44, 2, '#5a3c24');
      // a little potted plant on the sill
      px(28, 60, 8, 6, C.pot); px(30, 52, 4, 8, C.plant); px(27, 54, 4, 6, C.plant); px(33, 54, 4, 6, C.plant);

      // posters on the wall (Snake Skins)
      drawPoster(110, 24);
      drawPoster(180, 28);

      // wall shelf with Lilith's little tank + a lamp
      px(214, 40, 80, 4, C.deskDark);
      // lamp
      ctx.save(); ctx.globalAlpha = 0.6; var lg = ctx.createRadialGradient(232, 36, 0, 232, 36, 26); lg.addColorStop(0, C.lampGlow); lg.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = lg; ctx.fillRect(206, 12, 52, 40); ctx.restore();
      px(229, 28, 6, 12, '#3a2a1a'); px(226, 22, 12, 7, C.lamp);
      // tiny snake tank
      px(258, 26, 30, 14, '#2a4a44'); px(260, 28, 26, 10, '#3a6a5e');
      var sw = Math.sin(animT * 2) * 2;
      px(262, 34 + sw, 20, 2, '#f2efe6'); px(280, 32 + sw, 4, 2, '#f2efe6');
      px(258, 26, 30, 2, '#1a2a26');

      // the DESK
      px(96, 150, 130, 8, C.deskTop);
      px(96, 158, 130, 30, C.desk);
      px(100, 158, 4, 30, C.deskDark); px(218, 158, 4, 30, C.deskDark);
      // desk lamp glow over the book
      ctx.save(); ctx.globalAlpha = 0.5; var dg = ctx.createRadialGradient(150, 150, 0, 150, 150, 60); dg.addColorStop(0, C.lampGlow); dg.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = dg; ctx.fillRect(90, 96, 130, 80); ctx.restore();

      // the open journal on the desk (with flip animation)
      drawBook();

      // a sleepy cat curled on the corner of the desk
      drawCat(208, 146);

      // little pixel Shane sitting to the right, with his bass
      drawShane();

      // a coffee mug
      px(112, 142, 10, 8, '#d8d2c0'); px(120, 144, 4, 4, '#d8d2c0'); px(113, 140, 8, 2, '#fff');
      ctx.save(); ctx.globalAlpha = 0.4; px(114, 134 - (animT*4%6), 1, 3, '#cfcfcf'); px(117, 132 - (animT*4%6), 1, 3, '#cfcfcf'); ctx.restore();
    }

    function drawPoster(x, y) {
      px(x, y, 34, 46, '#2a1a2e'); px(x + 1, y + 1, 32, 44, '#1a1020');
      ctx.strokeStyle = 'rgba(231,200,121,0.5)'; ctx.lineWidth = 1; ctx.strokeRect(x + 0.5, y + 0.5, 33, 45);
      // snake squiggle
      ctx.strokeStyle = '#e85ee8'; ctx.lineWidth = 1.5; ctx.beginPath();
      ctx.moveTo(x + 8, y + 12); ctx.quadraticCurveTo(x + 26, y + 10, x + 12, y + 22); ctx.quadraticCurveTo(x + 6, y + 30, x + 24, y + 34); ctx.stroke();
      ctx.fillStyle = '#e7c24a'; ctx.font = 'bold 6px Oswald, sans-serif'; ctx.textAlign = 'center';
      ctx.fillText('SNAKE', x + 17, y + 42);
    }

    function drawBook() {
      var bx = 116, by = 130, bw = 88, bh = 22;
      // flip: scale the right page horizontally as it turns
      px(bx, by, bw, bh, C.book);
      px(bx, by, bw, 2, '#e2dabf'); px(bx, by + bh - 2, bw, 2, '#d8cfae');
      px(bx + bw / 2 - 1, by, 2, bh, C.bookEdge);       // spine
      // ruled lines
      for (var ly = by + 5; ly < by + bh - 3; ly += 5) px(bx + 4, ly, bw - 8, 1, 'rgba(80,90,120,0.25)');
      // ribbon bookmark
      px(bx + bw - 14, by - 4, 3, 10, C.ribbon);
      // flip page sweeping across
      if (flip > 0) {
        var w = Math.abs(Math.cos(flip * Math.PI)) * (bw / 2 - 2);
        px(bx + bw / 2, by, w, bh, flip > 0.5 ? '#efe7d0' : '#e6dcc0');
        px(bx + bw / 2, by, 1, bh, C.bookEdge);
      }
    }

    function drawShane() {
      var x = 250, baseY = 150, bob = Math.sin(animT) * 1;
      // stool
      px(x - 8, baseY, 16, 6, C.deskDark);
      // legs
      px(x - 6, baseY - 8, 4, 8, '#2a2a30'); px(x + 2, baseY - 8, 4, 8, '#2a2a30');
      // torso
      px(x - 7, baseY - 22 + bob, 14, 16, C.shirt);
      px(x - 7, baseY - 22 + bob, 14, 3, C.shirtHi);
      // head
      px(x - 5, baseY - 33 + bob, 10, 11, C.skin);
      // brown hair + ponytail
      px(x - 6, baseY - 35 + bob, 12, 4, C.hair);
      px(x - 6, baseY - 33 + bob, 2, 6, C.hair); px(x + 4, baseY - 33 + bob, 2, 5, C.hair);
      var pd = (gaze === 'away') ? -1 : 1;
      px(x + pd * 6, baseY - 33 + bob, 3, 9, '#5a3620');     // ponytail
      // face — looks away (down at bass) or toward you when looking
      if (gaze === 'looking') { px(x - 3, baseY - 29 + bob, 2, 2, '#1a1a1a'); px(x + 1, baseY - 29 + bob, 2, 2, '#1a1a1a'); }
      else { px(x - 2, baseY - 28 + bob, 4, 1, '#1a1a1a'); }
      // bass across the lap
      ctx.save(); ctx.translate(x, baseY - 8); ctx.rotate(-0.4);
      px(-2, -3, 20, 4, '#caa24a'); px(15, -5, 7, 8, '#caa24a'); ctx.restore();
      // status bubble
      if (gaze === 'stir') { px(x - 2, baseY - 44 + bob, 3, 3, '#e7c24a'); }
      if (gaze === 'looking') { ctx.fillStyle = '#ff6a52'; ctx.font = 'bold 11px Oswald'; ctx.textAlign = 'center'; ctx.fillText('!', x, baseY - 40 + bob); }
    }

    function drawCat(x, y) {
      var br = Math.sin(animT * 1.5) * 0.5;
      px(x, y + 2, 18, 8, C.catO);                 // body
      px(x + 14, y - 2, 7, 7, C.catO);             // head
      px(x + 14, y - 4, 2, 3, C.catO); px(x + 19, y - 4, 2, 3, C.catO); // ears
      px(x - 2, y + 3, 4, 3, C.catO);              // tail
      px(x + 16, y, 1, 1, C.catD); px(x + 19, y, 1, 1, C.catD);         // eyes (closed-ish)
      // sleepy z
      ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.font = 'bold 7px Oswald'; ctx.textAlign = 'left';
      ctx.fillText('z', x + 22, y - 4 + br * 2);
    }
    ...
    function update(dt) {
      animT += dt * 2;
      if (flip > 0) {
        flip = Math.max(0, flip - dt * 2.4);
        if (flip <= 0.5 && !swapped && pendingEntry) { applyEntry(pendingEntry); pendingEntry = null; swapped = true; }
        if (flip === 0) swapped = false;
      }
      // gaze cycle
      gazeTimer -= dt * 1000;
      if (gazeTimer <= 0) {
        if (gaze === 'away') setGaze('stir');
        else if (gaze === 'stir') setGaze(Math.random() < 0.7 ? 'looking' : 'away');
        else { /* was looking and player didn't shut -> caught only if they try to read; looking simply times out to away */ setGaze('away'); }
      }
      // slowly bleed deep meter if idle a long time? keep it simple: deep only grows.
    }
    function frame(t) {
      if (!playing) { return; }
    ...
    /* ---- input ---- */
    turnBtn.addEventListener('click', function (e) { e.preventDefault(); turnPage(); });
    shutBtn.addEventListener('click', function (e) { e.preventDefault(); shutBook(); });
    startBtn.addEventListener('click', start);
    // keyboard niceties
    document.addEventListener('keydown', function (e) {
      if (!playing) return;
      if (e.key === ' ' || e.key === 'ArrowRight' || e.key === 'Enter') { e.preventDefault(); turnPage(); }
      else if (e.key === 'ArrowDown' || e.key === 'Escape') { e.preventDefault(); shutBook(); }
    });
    document.addEventListener('visibilitychange', function () { if (document.hidden) pauseLoop(); else if (playing && !rafId) startLoop(); });

    // first paint behind overlay
    applyEntry({ tag: '\u2014', text: "(his journal, open on the desk)", date: "\u2014 private. obviously.", rarity: 'common' });
    reads = 0; readCt.textContent = '0'; deep = 0;
    try { draw(); } catch (e) {}
  }

  function boot() {
    build();
    var r0 = document.getElementById('sjGame');
    if (!r0 || !r0._init) { var n = 0; var iv = setInterval(function () { build(); var r = document.getElementById('sjGame'); if ((r && r._init) || ++n > 20) clearInterval(iv); }, 150); }
  }
